...
AASDM_data = read.csv("C:\\Users\\jelane\\Google Drive\\MODRN Stuff\\Models\\analyticSDM\\analyticSDM3 - Capped3\\analyticSDM3 - Capped3\\AnalyticAtheist_20200112_w_edu.csv")
...
trimmed_data <- AASDM_data[,c(2:52)]
res <- cor(trimmed_data)
round(res, 2)

important_data <- AASDM_data[,c(3:27)]

library(xtable)
library(Hmisc)
icor<-round(cor(trimmed_data),2)
upper<-icor
...
linReg_GB1 <- lm(godBelief ~ PRESENCE_OF_A + FIRST_TRAUMA +	FREQ_OF_TRAUMA +
                   ...
                   factual_information +	ontological_confusion +	analytical_thinking_style + supernaturalBelief, data=AASDM_data)
summary(linReg_GB1)
lm.beta(linReg_GB1)
...
linReg_GB1_1 <- lm(godBelief ~ PRESENCE_OF_A + FIRST_TRAUMA +	FREQ_OF_TRAUMA +
                   ...
                   INTUITIVE_THINKING_STYLE +	COGNITIVE_REFLECTION +	NEED_FOR_COGNITION +
                   ENV_RELIGIOUS_IMPORTANCE +	fact_resistance +	cognitive_inhibition +
                   factual_information +	ontological_confusion +	supernaturalBelief, data=AASDM_data)
summary(linReg_GB1_1)
lm.beta(linReg_GB1_1)
...
linReg_GB1_t <- lm(godBelief ~ INTELLIGENCE +	INIT_COG_INHIBITION +	COG_INHIB_DEPLETION +
                     ...
                     ENV_RELIGIOUS_IMPORTANCE +	fact_resistance +	cognitive_inhibition +
                     ontological_confusion, data=AASDM_data)
summary(linReg_GB1_t)
lm.beta(linReg_GB1_t)
...
linReg_SB1_st_sc <- lm(scale(supernaturalBelief) ~ scale(INTELLIGENCE) + 
                        ...
                        scale(ontological_confusion), data=AASDM_data)
summary(linReg_SB1_st_sc)
lm.beta(linReg_SB1_st_sc)
...
linReg_cog_ont <- lm(cognitive_inhibition ~ ontological_confusion, data=AASDM_data)
...
total := c+(a*b)
'
fit1 <- sem(modelm, se = "bootstrap", estimator = "GLS", data= AASDM_data)
summary(fit1)
...
linReg_GB2 <- lm(godBelief ~ PRESENCE_OF_A + FIRST_TRAUMA +	FREQ_OF_TRAUMA +
                   ...
                   INTUITIVE_THINKING_STYLE +	COGNITIVE_REFLECTION +	NEED_FOR_COGNITION +
                   ENV_RELIGIOUS_IMPORTANCE +	fact_resistance +	cognitive_inhibition +
                   factual_information +	ontological_confusion +	analytical_thinking_style, data=AASDM_data)
summary(linReg_GB2)
...
linReg_SB1 <- lm(supernaturalBelief ~ PRESENCE_OF_A + FIRST_TRAUMA +	FREQ_OF_TRAUMA +
                   ...
                   INTUITIVE_THINKING_STYLE +	COGNITIVE_REFLECTION +	NEED_FOR_COGNITION +
                   ENV_RELIGIOUS_IMPORTANCE +	fact_resistance +	cognitive_inhibition +
                   factual_information +	ontological_confusion +	analytical_thinking_style + godBelief, data=AASDM_data)
summary(linReg_SB1)
...
#trimmed_data_gbelievers <- AASDM_data
trimmed_data_gbelievers$godCat <- factor(ifelse(AASDM_data$godBelief > 0, "believer", "nonbeliever"))
...
library(Matrix)
library(data.table)
trimmed_data2 <- AASDM_data[, c(2:52)]
